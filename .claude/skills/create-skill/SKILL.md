...
1. **Check for duplicates.** Run `ls .claude/skills/` and scan names. Don't create a skill that overlaps with an existing one. If similar, update the existing skill instead.
...
3. **Create a directory** at `.claude/skills/{name}/` and write `SKILL.md` inside it with this template:
...
4. **Validate the skill:**
   ...
   - The trigger should be unambiguous
...
- **Skill chaining:** Skills can reference other skills in their steps

## Notes

- Created: 2026-03-22
- Origin: Phase 3E skills system, updated for SDK-native discovery in v2
